...
class FastTranslatorFastPrefix(FastTranslator):
    """
    ...
    def _translate_batch(self, batches, sub_batches, prefix_tokens=None):
        ...
        max_len = 512 #self.opt.max_sent_length
        ...
        if self.opt.sub_model:
            for i in range(self.n_sub_models):
                sub_decoder_states[i] = self.sub_models[i].create_decoder_state(sub_batches[i], beam_size, type=2,
                                                                                buffering=self.buffering)
        logging.info("encoding time: %.3f" % (time.time() - enc_time))
        if self.dynamic_max_len:
            src_len = src.size(0)
            max_len = math.ceil(int(src_len) * self.dynamic_max_len_scale)

        decoding_time = time.time()
        decoding_time_no_prefix = time.time()

        feed_in_tokens = tokens.clone()
        start = 0
        if prefix_tokens is not None and prefix_tokens.size(1) > 0:
            preftime = time.time()
            start = prefix_tokens.size(1)
            feed_in_tokens[:, 1:start+1] = prefix_tokens
            # for i in range(start):
            #     decode_input = feed_in_tokens[:, :i + 1]
            #     _, _ = self._decode(decode_input, decoder_states,
            #                                             sub_decoder_states=sub_decoder_states)
            decode_input = feed_in_tokens[:, :start]
            _, _ = self._decode(decode_input, decoder_states,
                                                         sub_decoder_states=sub_decoder_states, fill_cache=True)
            prefix_tokens = None
            logging.info('prefix time: %.3f' % (time.time() - preftime))
        max_len -= start
        # Start decoding
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            decode_input = feed_in_tokens[:, :start + step + 1]
            ...
            feed_in_tokens[:, start+1+step] = tokens[:,step + 1]
        ...
        logging.info("decoding time: %.2f" % (time.time() - decoding_time))
        return finalized, gold_scores, gold_words, allgold_scores

    def _decode(self, tokens, decoder_states, sub_decoder_states=None, fill_cache=False):
        ...
        for i in range(self.n_models):
            ...
            decoder_output = self.models[i].step(tokens, decoder_states[i])#, fill_cache=fill_cache)

            outs[i] = decoder_output['log_prob']
            attns[i] = decoder_output['cross_contribution']

        for j in range(self.n_sub_models):
            sub_decoder_output = self.sub_models[j].step(tokens, sub_decoder_states[j], fill_cache=fill_cache)
    ...
    def build_prefix(self, prefixes, bsz=None):
        ...
        if self.external_tokenizer is None:
            ...
            #prefix_data = [torch.LongTensor(self.external_tokenizer(sent)['input_ids'])[:-1]
                            #for sent in prefixes]
            prefix_data = [sent
                            for sent in prefixes]
    ...
    def translate(self, src_data, tgt_data, past_src_data=None, sub_src_data=None, type='mt', prefix=None):
        ...
        if prefix is not None:
            ...
            print('prefix tensor', prefix_tensor)
        else:
            prefix_tensor = None
        logging.info("Prefix time: {}".format(time.time() - prefix_time))
        ...
        logging.info("translation time: %.2f seconds" % (time.time() - translate_time))
